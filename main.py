...
while(True):
    if (num_items(Items.Hay) < config.Min_Hay
        ...
        # 都不缺了就随便来一个
        random_work_list = [(work.try_planting_common_crops, util._always_true),
                       (work.try_planting_sunflower, work.harvest_sunflower_max),
                       (work.try_planting_pumpkin, work.try_harvest_pumpkin)]
        random_work = util.random_elem(random_work_list)
        work.inspection(random_work[0], random_work[1])
